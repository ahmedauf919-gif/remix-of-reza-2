...
const NARRATION: readonly string[] = [
  ...
  "TAQA funds, builds, owns and operates the water plant, so the farm pays only for the water it uses, with little to no upfront capital cost — covering brackish or seawater reverse-osmosis desalination units, pre-treatment and filtration, storage reservoirs and pumping, plus irrigation distribution and smart meters. TAQA first studies the farm's water source, salinity and demand, then treats brackish or seawater through reverse osmosis to irrigation quality before storing, pumping and distributing it to the fields — operating the plant continuously and guaranteeing both volume and quality.",
  ...
  "TAQA funds, builds, owns and operates the battery system, so the farm pays only for the service it receives, with little to no upfront capital cost — covering battery energy-storage units, a power-conversion system and inverters, switchgear and the grid or solar interface, plus energy-management and SCADA controls. TAQA sizes the storage to the farm's load and solar profile, then installs it so the system charges from surplus daytime solar and discharges to run pumps and cold storage after dark — the energy-management system optimizing every charge and discharge cycle.",
  ...
  // 21 Mobile CNG: Scope
  "TAQA funds, builds, owns and operates the gas supply chain, so the farm pays only for the gas it uses, with little to no upfront capital cost — covering a mother station and compression, CNG virtual-pipeline trailers, on-site decompression and a PRMS skid, plus metering and safety controls. Gas is compressed at the mother station to around 250 bar, trailered to the farm as a virtual pipeline, then decompressed on-site to process pressure before it fuels dryers, greenhouses and processing, with TAQA tracking usage and refilling ahead of demand.",
