...
const NARRATION: readonly string[] = [
  ...
  // 6 — Mobile CNG: Scope
  "TAQA funds, builds, owns and operates the mobile CNG asset — the mother station and compression, the CNG and virtual-pipeline trailers, on-site decompression and PRMS skid, and industrial metering and controls. Gas is compressed to roughly 250 bar at the mother station, trucked to the plant as a virtual pipeline, then decompressed on-site to feed boilers, furnaces and process lines. TAQA tracks usage and refills ahead of demand, so the factory pays only for the gas it consumes, with little to no upfront capital.",
  ...
  "TAQA designs the zone's power network around actual factory load demands, then builds and energizes the HV and MV substations, switchgear, distribution transformers, cabling and protection equipment. Smart meters bill each factory individually for its own consumption, and a 24/7 control room keeps the entire zone powered, balanced and monitored. The result is a licensed industrial power network — already delivering 450 MVA across zones — with reliable supply, metering, billing and operations all handled by one accountable party.",
